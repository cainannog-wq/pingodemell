...
4. Checkout e registro do pedido — formulário com dados do cliente (nome, WhatsApp, email, ocasião, modo e endereço de entrega, data e hora, forma de pagamento, observações), grava o pedido na tabela de pedidos do Supabase sempre pelo servidor (POST /api/pedidos; nenhum papel da API grava direto na tabela; leitura e update restritos a usuário autenticado) e, na sequência, abre o WhatsApp com a mensagem formatada. O pedido nasce do clique de confirmação no site, não da leitura da mensagem enviada. Pedir junto: teste automatizado confirmando o servidor gravando o pedido e gravação/leitura/update anônimos direto pela API bloqueados, mais validação de campo obrigatório e rate limit por IP no backend contra spam ou flood, sem CAPTCHA visível nessa tela
...
- A cliente pede sem login, mas o pedido só é gravado pelo servidor (POST /api/pedidos); a tabela de pedido não aceita gravação direta de nenhum papel da API (mudança de 24/09/2026, ver abaixo). A proteção contra spam ou flood é validação de campo obrigatório mais rate limit por IP no backend, sem CAPTCHA visível na tela de checkout, pra não acrescentar fricção pro cliente final
...
- **Foto de referência do bolo personalizado**: ainda em aberto em 14/09/2026, precisa da resposta da cliente. Se for por upload no site, a imagem fica junto do pedido gravado (aparece no histórico e no detalhe do pedido no painel), mas exige endurecer a proteção da gravação de pedido sem login (feita pelo servidor) (tipo e tamanho de arquivo, limite de volume) além do que já existe hoje (rate limit por IP e campo obrigatório, sem CAPTCHA). Se for pelo WhatsApp, a foto fica fora do registro no Supabase e não aparece no painel.
...
## Mudança registrada em 24/09/2026 — gravação de pedido só pelo servidor

Até aqui a tabela de pedidos aceitava insert direto de qualquer visitante pela API do Supabase, com a chave anônima que está no código do site. Com isso, o limite por IP do servidor não protegia nada: bastava pular o servidor e gravar direto. Decisão do Cainan no PR seguranca-api: a cliente continua pedindo sem login, mas o pedido só é gravado pelo servidor (POST /api/pedidos, com validação e limite por IP); nenhum papel da API (visitante anônimo ou admin logado) grava direto na tabela. O teste que antes provava "insert anônimo funcionando" passa a provar o contrário: o anônimo é recusado e o servidor grava. Consequência futura: quando existir "admin abre pedido fora do limite de data", esse pedido também é gravado pelo servidor.
